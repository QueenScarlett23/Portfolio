...
public:
    ...
    Model(string const& path, glm::vec3 pos = glm::vec3(0), float sc = 1.0f, glm::vec3 rot = glm::vec3(0), bool gamma = false) : gammaCorrection(gamma)
    {
        position = pos;
        scale = glm::vec3(sc);
        rotation = rot;
    ...
    long Draw(Shader& shader)
    {
        long verts = 0;
        for (unsigned int i = 0; i < meshes.size(); i++)
            verts += meshes[i].Draw(shader);
        return verts;
    }
